# 训练字符级别的语言模型，使用《神雕侠侣》作为语料，期望让模型编写武侠小说
本项目主要包含三个模块：
...
```
Reading model parameters from train_dir/novel/generate.ckpt-46000
...
问题跟踪与改进
<u>一、问题一：训练时样本的生成方式以及指定的样本长度对模型训练结果的影响（本问题中的实验语料为诗词语料）</u>
...
Restored from: train_dir/poetry/model-300
> 锡
山山人远，一人一
人。
云人，无水，风风。
...
Restored from: train_dir/poetry/model-2800
> <unk><unk>，<unk><unk>，山花。
山。
何日，山色，无人。
何。
...
Restored from: train_dir/poetry/model-7100
> 巫峡日，风色，寒风。
江色，秋色，寒风。
云。
云中一一客，秋水。
...
```
(1)中由于使用的训练数据长度太短，模型没有学习到长依赖，所以不能生成合理的结果；
(2)中使用的训练数据长度比较大，模型有效的学习到了长依赖，所以生成了合理的结果。
```



2、使用循环的样本生成方式：一个batch中，targets完全是由inputs右移一位得到的，不进行循环。仍然进行试验：
（1）设置训练数据的max_time为10，经过不同次数的训练后，生成数据的结果为：

```
global step 300 learning rate 0.0050 step-time 0.09 perplexity 249.15
Reading model parameters from train_dir/poetry/generate.ckpt-300
...
```
global step 1600 learning rate 0.0050 step-time 0.08 perplexity 182.27
Reading model parameters from /Users/baihai/projects/char_lm/train_dir/poetry/generate.ckpt-1600
> 知不自不难。
何日何人见，不得一云人。
风中一水日，寒云入山人。
山山无日去，天日有云山。
...
```
global step 3900 learning rate 0.0025 step-time 0.09 perplexity 135.75
Reading model parameters from /Users/baihai/projects/char_lm/train_dir/poetry/generate.ckpt-3900
> 一子何人别。
一日何人事，无君不得人。
不知何处事，不得一君期。
一日何时去，无人见不稀。
...
Reading model parameters from train_dir/poetry/generate.ckpt-1600
> 有人不可思。
不有何处处，无处自相归。
一君何日去，何处不相知。
不有不相去，何日不相思。
...
Reading model parameters from train_dir/poetry/generate.ckpt-2500
> 有人何处来。
一心无处在，一去一年年。
日日无时去，春来不有心。
山山不不见，何处不知愁。
...
<u>二、计算损失函数时，如果使用完整的softmax，由于字符数目比较大，会导致训练速度比较慢；所以把损失函数修改为sample softmax，从而提高训练速度。（本问题中的实验语料为诗词语料）（具体的关于candidate sampling的知识，请参考tensorflow的教程）</u>
（1）原来的训练速度：
...
三、在使用《神雕侠侣》进行训练时，训练过程中仍然需要自适应地修改学习率，否则模型可能难以收敛（发生振荡）。另外，在训练过程中样本长度会不停地变化，使模型可以学习到不同长度的依赖关系，适应不同长度的语句风格。